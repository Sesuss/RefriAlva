...
module.exports={
    ...
    async img(req,res){
        ...
        let Restante = datos[0].CostoServicio-datos[0].Presupuesto
        ...
        if (garantia[0].FechaGarantia =="2021-12-31") {
            res.render("factura_garantia.hbs",{ layout:"mainpdf",datos,cliente,tecnico,equipo,Restante})
            
        }else{

            res.render("factura.hbs",{ layout:"mainpdf",datos,cliente,tecnico,equipo,Restante})
        }
    ...
    async desimg(req,res){
        await crearimagen("http://localhost:3303/ver")
        let idOrden = await pool.query("SELECT * FROM `tblidnotas`")
        idOrden=idOrden[0].IdOrden
    ...
    async despdf(req,res){
        const pdf = await crearpdf("http://localhost:3303/verpdf")
